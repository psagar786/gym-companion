...
function workoutView() {
  ...
  app.innerHTML = `<main class="shell">${previewBanner}<button class="link-button" data-preview-back-member>‹ All days</button><section class="detail-head"><div><p class="eyebrow">${routine[selectedDay].day}</p><h1>${escapeHtml(plan.focus)}</h1><p>${Object.values(checks).filter(Boolean).length}/${plan.slots.length} complete</p></div></section><section class="workout-list">${plan.slots.map((slot, index) => { const movement = exercise(slot.primary); return `<article class="card exercise"><div class="visual" style="background-image:url('${escapeHtml(movement?.image || '')}')"></div><div><h3>${escapeHtml(movement?.name || 'Exercise')}</h3><p>${escapeHtml(movement?.scheme || '')}</p><p>${escapeHtml(movement?.cue || '')}</p></div><label class="check"><input type="checkbox" data-preview-check="${index}" ${checks[index] ? 'checked' : ''}><span>Done</span></label></article>`; }).join('')}</section></main>`;
}
