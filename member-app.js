...
function effectiveTemplateKey() { return 'periodized-abc'; }
function currentTemplate() { return effectiveTemplateKey()==='periodized-abc' ? { name:'3-Phase Progression Plan', days:periodized.days.filter(day=>day.weekKey==='A'&&day.dayIndex<6) } : effectiveTemplateKey()==='threeweek-ppl' ? { name:v53Plan.name||v53Plan.label||'3-Week PPL Rotation', days:v5Routine } : effectiveTemplateKey()==='biweekly' ? { name:'Bi-Weekly Activity', days:[] } : (training.templates[effectiveTemplateKey()] || training.templates.v5ppl); }
...
function memberDayPresentation(dayIndex, plan=displayPlan(dayIndex)) {
  ...
  return {focusLabel:memberDayFocusLabels[dayIndex]||'Training session',exerciseCount:planSlots(plan).length,progressionLabel};
}
function memberSnapshotPresentation(session) {
  ...
  return {focusLabel:session?.plan_snapshot?.focus||'Workout',progressionLabel:''};
}
function applyMemberPresentation() {
  if (effectiveTemplateKey()!=='periodized-abc') return;
  if (state.screen==='workout') {
    const plan=displayPlan(state.dayIndex), presentation=memberDayPresentation(state.dayIndex,plan), title=document.querySelector('.detail-head h1');
    if (title) title.textContent=presentation.focusLabel;
    document.querySelectorAll('.rotation-badge').forEach(node=>node.remove());
    const detailHead=document.querySelector('.detail-head > div');
    if (detailHead && presentation.progressionLabel && !detailHead.querySelector('.progression-nudge')) {
      const nudge=document.createElement('small'); nudge.className='progression-nudge'; nudge.textContent=presentation.progressionLabel; detailHead.appendChild(nudge);
    }
    const outlookTitle=document.querySelector('.outlook-card h2');
    if (outlookTitle) outlookTitle.textContent=`What ${presentation.focusLabel} can build`;
    const optionalNote=document.querySelector('.optional-picker .muted');
    if (optionalNote) optionalNote.textContent='Only compatible options for this day are shown.';
  }
  if (state.screen==='plan') {
    const heading=document.querySelector('.section-title h1'); if (heading) heading.textContent='Set up your progression plan';
    const option=document.querySelector('select[name="template_key"] option'); if (option) option.textContent='3-Phase Progression Plan';
    const templateNote=document.querySelector('select[name="template_key"]')?.closest('label')?.nextElementSibling; if (templateNote) templateNote.textContent='Exercises rotate across three progression phases. Saved history never changes.';
  }
  if (state.screen==='history') {
    const session=calendarSessionForDate(state.historyDate), presentation=memberSnapshotPresentation(session), paragraphs=document.querySelectorAll('.detail-hero p');
    if (paragraphs[1]) paragraphs[1].textContent=presentation.focusLabel;
  }
  if (state.screen==='profile') {
    const sessions=visibleSessions(); document.querySelectorAll('.history-button').forEach((row,index)=>{ const presentation=memberSnapshotPresentation(sessions[index]); const label=row.querySelector('b'); if(label) label.textContent=presentation.focusLabel; });
  }
}
...
function renderHome() {
  ...
  const selectedTemplate=currentTemplate().name;
  ...
  app.innerHTML = `<main class="shell">${header()}${banner()}${notice()}<section class="home-hero home-hero-clean"><p class="eyebrow">${dayName}</p><h1>Your weekly training</h1><p>${escapeHtml(selectedTemplate)} · ${escapeHtml(tier)}</p></section><section class="section-title"><div><p class="eyebrow">THIS WEEK</p><h2>Choose a day</h2></div></section><section class="day-grid">${routine.map((fallback,index) => { const plan=displayPlan(index), presentation=memberDayPresentation(index,plan), date=scheduledDate(index), isToday=index===todayIndex; return `<button class="day-card ${isToday?'is-today':''}" data-day="${index}" data-screen="workout" ${isToday?'aria-current="date"':''}><span class="day-card-heading"><b>${escapeHtml(fallback.day)}</b><time>${date.toLocaleDateString(undefined,{day:'numeric',month:'short'})}</time>${isToday?'<span class="today-badge">Today</span>':''}</span><span>${escapeHtml(presentation.focusLabel)}</span><small>${presentation.exerciseCount} main exercises</small></button>`; }).join('')}</section>${calendarMarkup(true)}</main>`;
}
...
function guided(title, items, checks, key, options=[]) {
  if (!items?.length) return '';
  const isTendon=key==='tendon', isRequired=!isTendon;
  const visible=items.slice(0,2), remaining=items.slice(2);
  const cards=list=>list.map((raw,index)=>{const item=prescribeExercise({...raw,name:raw.title||raw.name,scheme:raw.scheme||raw.duration,prescriptions:raw.prescriptions});const prescription=item.tierPrescription||tierPrescription(item);const sourceIndex=items.indexOf(raw);return `<article class="exercise guided-exercise"><button type="button" class="visual-button" data-detail-key="${escapeHtml(slugify(item.name||''))}" aria-label="View details for ${escapeHtml(item.name||'exercise')}">${imageMarkup(item,'visual')}</button><div class="exercise-copy"><label class="check card-done"><input type="checkbox" data-check="${key}" data-index="${sourceIndex}" ${checks?.[sourceIndex]?'checked':''}><span>Done</span></label><span class="priority-badge">${sourceIndex===0?'Priority 1':sourceIndex===1?'Priority 2':'Additional'}</span><h3>${escapeHtml(item.name)}</h3><p class="exercise-dose">${escapeHtml(prescription.displayDose)}</p><p class="exercise-rest">${escapeHtml(prescription.displayRest)}</p><button class="link-button detail-button" data-detail-key="${escapeHtml(slugify(item.name||''))}">View exercise details →</button></div></article>`;}).join('');
  const body=`<div class="guided-scroll">${cards(visible)}${remaining.length?`<div class="guided-more">${cards(remaining)}</div>`:''}</div>${remaining.length?'<p class="guided-nudge">Scroll for more</p>':''}${options.length?`<details class="guided-options"><summary><span>Need a different option?</span><span class="guided-toggle">View</span></summary>${optionalCards(options, 'guided')}</details>`:''}`;
  const heading=`<span><p class="eyebrow">${isTendon?'OPTIONAL PREPARATION':'REQUIRED GUIDED WORK'}</p><h2>${title}</h2><p class="muted">${isTendon?'Tap to expand when you want tendon preparation.':'Complete these before moving on.'}</p></span><span class="guided-toggle">${isTendon?'Expand':'Open'}</span>`;
  return isTendon?`<details class="guided-panel guided-tendon"><summary>${heading}</summary>${body}</details>`:`<section class="guided-panel guided-required"><div class="guided-heading">${heading}</div>${body}</section>`;
}
...
function renderWorkout() {
  ...
  const presentation=memberDayPresentation(state.dayIndex,plan);
...
function renderPlan() {
  ...
  const planDays=key==='periodized-abc' ? (periodized.days||[]).filter(day=>day.weekKey===periodizedWeekKey()&&day.dayIndex<6).filter((day,index,all)=>all.findIndex(other=>other.dayIndex===day.dayIndex)===index).slice(0,6) : key==='biweekly' ? (biweekly.days||[]).filter(day=>day.weekKey===biweeklyWeekKey()&&day.dayIndex<6) : (template.days||v5Routine);
  app.innerHTML=`<main class="shell">${header()}${banner()}${notice()}<button class="link-button" data-screen="home">‹ Home</button><section class="section-title"><div><p class="eyebrow">MY PLAN</p><h1>Build your A–B–A–C week</h1><p>Your selected level applies to future workouts; saved history never changes.</p></div></section><section class="card"><form id="training-preferences" class="stack"><label class="field">Training template<select name="template_key"><option value="periodized-abc" selected>Periodized A–B–A–C</option></select></label><p class="muted">Week A → Week B → Week A → Week C. This is the only active member plan.</p><label class="field">Training level<select name="tier" data-tier-select>${Object.entries(training.levels).map(([itemKey,value])=>`<option value="${itemKey}" ${tier===itemKey?'selected':''}>${value.label} — ${value.subtitle}</option>`).join('')}</select></label><div data-tier-preview>${tierPreviewMarkup(tier)}</div><div data-future-plan-preview>${futurePlanPreviewMarkup('periodized-abc',tier)}</div><button class="button">Apply my future plan</button></form></section><section class="section-title"><div><p class="eyebrow">${activeDay?.day||'Day'} OPTIONS</p><h2>Optional extras</h2><p>Only related muscle groups are shown. Tap ＋ Add to save up to two.</p></div></section><section class="card stack">${optionalCards(planCandidates)}${extras.map((item,index)=>`<article class="compact-row"><span>${escapeHtml(item.exercise?.name||'Extra exercise')}</span><button type="button" class="pill" data-remove-extra="${index}">− Remove</button></article>`).join('')||'<p class="muted">No extras saved for this day yet.</p>'}</section><div class="day-picker">${planDays.map((day,index)=>`<button class="day-chip ${state.dayIndex===index?'selected':''}" data-plan-day="${index}">${(day.day||day.dayName||['Mon','Tue','Wed','Thu','Fri','Sat'][index]||'Day').slice(0,3)}</button>`).join('')}</div></main>`;
}
...
function render(){ try { if(!state.user) return renderSignIn(); if(new URLSearchParams(location.search).get('qa')==='art-v2') return renderV2PilotGallery(); if(state.screen==='detail') return renderDetail(); if(state.screen==='history'){renderHistory();applyMemberPresentation();return;} if(state.screen==='workout'){renderWorkout();applyMemberPresentation();return;} if(state.screen==='plan'){renderPlan();decoratePlan();applyMemberPresentation();return;} if(state.screen==='artwork') return renderArtworkReview(); if(state.screen==='habits') return renderHabits(); if(state.screen==='profile'){renderProfile();applyMemberPresentation();return;} renderHome();applyMemberPresentation(); } catch(error) { console.error('[Fitness 7] Render error', error); app.innerHTML=`<main class="shell"><div class="notice">Unable to render this screen. Return home and try again.</div><button class="button" data-screen="home">Return home</button></main>`; } }
