...
export function validateRoutine(plan, options = {}) {
  ...
  if (options.runtime) {
    ...
    for (const name of ['hasWorkoutActivity','ringStatus','zoneCounts']) if (!body(name).includes('tendon')) add('TENDON_PROGRESS', `member-app.js:${name}`, `${name} does not include tendon progress in warm-up/activity accounting.`);
